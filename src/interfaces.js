...
export function NetworkManagerModel() {
    ...
    self.preinit = new Promise((resolve, reject) => {
        client
                ...
                .then((reply, options) => {
                    if (options.flags) {
                        if (options.flags.indexOf('>') !== -1) utils.set_byteorder('be')
                        else if (options.flags.indexOf('<') !== -1) utils.set_byteorder('le')
                        resolve()
                    }
                })
                .catch(complain)
    ...
    function get_object(path, type) {
        ...
        function Constructor() {
            ...
            priv(this).type = type
            priv(this).path = path
            for (const p in type.props) this[p] = type.props[p].def
        }
        if (!objects[path]) {
    ...
    function objpath(obj) {
        if (obj && priv(obj).path) return priv(obj).path
        else return '/'
    }
    ...
    self.synchronize = function synchronize() {
        if (outstanding_refreshes === 0) {
            ...
            if (!export_model_deferred) export_model_deferred = cockpit.defer()
    ...
    function onNotifyEventHandler(event, data) {
        Object.keys(data).forEach((path) => {
            ...
            Object.keys(interfaces).forEach((iface) => {
                ...
                if (props) interface_properties(path, iface, props)
                else interface_removed(path, iface)
            })
        })
    }
    ...
    function settings_from_nm(settings) {
        function get(first, second, def) {
            if (settings[first] && settings[first][second]) return settings[first][second].v
            else return def
        }
    ...
    function settings_to_nm(settings, orig) {
        ...
        function set(first, second, sig, val, def) {
            ...
            if (val !== undefined) result[first][second] = cockpit.variant(sig, val)
            else delete result[first][second]
        }
        ...
        if (settings.ethernet) {
            ...
            set(
                ...
                's',
                settings.ethernet.assigned_mac_address
            )
    ...
    function device_type_to_symbol(type) {
        ...
        case 0:
            return 'unknown'
        case 1:
            return 'ethernet' // 802-3-ethernet
        case 2:
    ...
    function refresh_settings(obj) {
        push_refresh()
        client
                .call(objpath(obj), 'org.freedesktop.NetworkManager.Settings.Connection', 'GetSettings')
                .then(function (reply) {
    ...
    function refresh_udev(obj) {
        ...
        push_refresh()
        cockpit
                .spawn(['udevadm', 'info', obj.Udi], { err: 'message' })
                .then(function (res) {
                    const props = {}
        ...
        exporters: [
            ...
            function (obj) {
                ...
                function consider_for_main(con) {
                    if (
                        !obj.MainConnection ||
                        connection_settings(obj.MainConnection).timestamp <
                            connection_settings(con).timestamp
                    ) {
                ...
                if (
                    obj.Device &&
                    obj.Device.ActiveConnection &&
                    obj.Device.ActiveConnection.Connection
                ) {
...
function render_interface_link(iface) {
    return (
        <Button variant='link' tabindex='0' isInline onClick={() => cockpit.location.go([iface])}>
            {iface}
        </Button>
    )
}
    ...
    manager.checkpoint_create(options.devices || [], rollback_time).then(function (cp) {
        ...
        modify()
                ...
                .catch(function () {
                    ...
                    if (options.rollback_on_failure) manager.checkpoint_rollback(cp)
                    else manager.checkpoint_destroy(cp)
                })
    })
}
...
export function connection_devices(con) {
    ...
    if (con)
        con.Interfaces.forEach(function (iface) {
...
export function member_connection_for_interface(group, iface) {
    return (
        group &&
        array_find(group.Members, function (s) {
...
export function set_member(model, group_connection, group_settings, member_type, iface_name, val) {
    ...
    if (val) {
        ...
        if (main_connection) {
            ...
            if (
                member_settings.connection.group == group_settings.connection.uuid ||
...
export function apply_group_member(
    choices,
    model,
    apply_group,
    group_connection,
        ...
        if (
            ...
            Object.keys(choices)
                    ...
                    .forEach((choice) => {
                        ...
                        iface.Device.ActiveConnection.Connection
                        ) {
